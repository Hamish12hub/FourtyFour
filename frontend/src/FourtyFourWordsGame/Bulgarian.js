...
import theFlag from '../photos/flags/Bulgaria.png'
...
function Bulgarian() {
  ...
  const TheLanguage = "Bulgarian"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
'Zdraveite','zdrasti','Blagodarya ti','Molya te','I','The','da','Ne','Dobro utro','Dobur den',
//11 - 21
'Khrana','yaitsa','Mlyako','Khlyab','Chips','Riba','Shokolad','pitsa','voda','Bira','Vino',
//22-29
'Restorant','Kushta','plazh','Magazin','grad','Basein','Parkiraite','Toaletna',
//30-35
'Smetkata','Pari','Telefon','Vegetarianets','Vlak','Kola',
//36-44
"edin","dve","Tri","Chetiri","Pet","shest","Sedem","Osem","Devete"
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/bulgarian", "");
  ...
  const alertCACScore = async () => {
    ...
    const req = await fetch('http://localhost:1337/BulgarianScore', {
      headers: {'x-access-token': token},
    })
    const data = await req.json()
    if(data.status == "ok"){
      setOldScore(data.bulgarian)
    }else{
      alert('Invalid Token');
    }
  };
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingBulgarianScore', {
      method: "POST",
