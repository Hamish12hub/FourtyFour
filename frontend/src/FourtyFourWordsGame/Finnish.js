...
import theFlag from '../photos/flags/Finland.png';
...
function Finnish() {
  ...
  const TheLanguage = "Finnish"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
"Hei", "Moi", "Kiitos", "Ole hyva", "Ja", "Se", "Kylla", "Ei", "Hyvaa aamua", "Hyvaa iltapaivaa",
//11-21
"Ruoka", "Munat", "Maito", "Leipa", "Chipsit", "Kala", "Suklaa", "Pizza", "Vesi", "Olut", "Viini",
// 22-29
"Ravintola", "Talo", "Ranta", "Marketti", "Kaupunki", "Uima-allas", "Puisto", "WC",
//30-35
"Lasku", "Raha", "Puhelin", "Kasvissyoja", "Juna", "Auto",
//36-44
"Yksi", "Kaksi", "Kolme", "Nelja", "Viisi", "Kuusi", "Seitseman", "Kahdeksan", "Yhdeksan"
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/finnish", "");
  ...
  const alertCACScore = async () => {
    ...
    const req = await fetch('http://localhost:1337/FinnishScore', {
      headers: {'x-access-token': token},
    })
    const data = await req.json()
    if(data.status == "ok"){
      setOldScore(data.finnish)
    }else{
      alert('Invalid Token');
    }
  };
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingFinnishScore', {
      method: "POST",
