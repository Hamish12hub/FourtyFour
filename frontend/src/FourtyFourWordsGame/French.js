...
import theFlag from '../photos/flags/French.png';
...
function French() {
  ...
  const TheLanguage = "French"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
'Bonjour', 'Salut', 'Merci', 'S il vous plait', 'Et', 'La', 'Oui', 'Non', 'Bonjour', 'Bon apres-midi',
//11-21
'Nourriture', 'Oeufs', 'Lait', 'Pain', 'Chips', 'Poisson', 'Chocolat', 'Pizza', 'Eau', 'Biere', 'Vin',
// 22-29
'Restaurant', 'Maison', 'Plage', 'Magasin', 'Ville', 'Piscine', 'Parc', 'Toilette',
//30-35
'L baddition', 'Argent', 'Telephone', 'Vegetarien', 'Train', 'Voiture',
//36-44
'Un', 'Deux', 'Trois', 'Quatre', 'Cinq', 'Six', 'Sept', 'Huit', 'Neuf'
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/french", "");
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingFrenchScore', {
      method: "POST",
