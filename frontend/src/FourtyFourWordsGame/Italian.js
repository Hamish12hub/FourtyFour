...
import theFlag from '../photos/flags/Italian.png';
...
function Italian() {
  ...
  const TheLanguage = "Italian"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
'Ciao', 'Ciao', 'Grazie', 'Per favore', 'E', 'La', 'Si', 'No', 'Buongiorno', 'Buon pomeriggio',
//11-21
'Cibo', 'Uova', 'Latte', 'Pane', 'Patatine', 'Pesce', 'Cioccolato', 'Pizza', 'Acqua', 'Birra', 'Vino',
// 22-29
'Ristorante', 'Casa', 'Spiaggia', 'Negozio', 'Citta', 'Piscina', 'Parco', 'Bagno',
//30-35
'Il conto', 'Soldi', 'Telefono', 'Vegetariano', 'Treno', 'Auto',
//36-44
'Uno', 'Due', 'Tre', 'Quattro', 'Cinque', 'Sei', 'Sette', 'Otto', 'Nove'
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/italian", "");
  ...
  const alertCACScore = async () => {
    ...
    const req = await fetch('http://localhost:1337/ItalianScore', {
      headers: {'x-access-token': token},
    })
    const data = await req.json()
    if(data.status == "ok"){
      setOldScore(data.italian)
    }else{
      alert('Invalid Token');
    }
  };
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingItalianScore', {
      method: "POST",
