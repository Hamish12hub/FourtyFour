...
import theFlag from '../photos/flags/Poland.png';
...
function Polish() {
  ...
  const TheLanguage = "Polish"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
'Witaj', 'Czesc', 'Dziekuje', 'Prosze', 'I', 'Ta', 'Tak', 'Nie', 'Dzien dobry', 'Dobry wieczor',
//11-21
'Jedzenie', 'Jajka', 'Mleko', 'Chleb', 'Chipsy', 'Ryba', 'Czekolada', 'Pizza', 'Woda', 'Piwo', 'Wino',
// 22-29
'Restauracja', 'Dom', 'Plaza', 'Sklep', 'Miasto', 'Basen', 'Park', 'Toaleta',
//30-35
'Rachunek', 'Pieniadze', 'Telefon', 'Wegetarianin', 'Pociag', 'Samochod',
//36-44
'Jeden', 'Dwa', 'Trzy', 'Cztery', 'Piec', 'Szesc', 'Siedem', 'Osiem', 'Dziewiec'
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/polish", "");
  ...
  const alertCACScore = async () => {
    ...
    const req = await fetch('http://localhost:1337/PolishScore', {
      headers: {'x-access-token': token},
    })
    const data = await req.json()
    if(data.status == "ok"){
      setOldScore(data.polish)
    }else{
      alert('Invalid Token');
    }
  };
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingPolishScore', {
      method: "POST",
