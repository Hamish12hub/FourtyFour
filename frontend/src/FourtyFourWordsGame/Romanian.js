...
import theFlag from '../photos/flags/Romania.png';
...
function Romanian() {
  ...
  const TheLanguage = "Romanian"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
'Salut', 'Salut', 'Multumesc', 'Te rog', 'Si', 'Aceasta', 'Da', 'Nu', 'Buna dimineata', 'Buna ziua',
//11-21
'Mâncare', 'Oua', 'Lapte', 'Pâine', 'Chips uri', 'Peste', 'Ciocolata', 'Pizza', 'Apa', 'Bere', 'Vin',
// 22-29
'Restaurant', 'Casa', 'Plaja', 'Magazin', 'Oras', 'Piscina', 'Parc', 'Toaleta',
//30-35
'Contul', 'Bani', 'Telefon', 'Vegetarian', 'Tren', 'Masina',
//36-44
'Unu', 'Doi', 'Trei', 'Patru', 'Cinci', 'Sase', 'Sapte', 'Opt', 'Noua'
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/romanian", "");
  ...
  const alertCACScore = async () => {
    ...
    const req = await fetch('http://localhost:1337/RomanianScore', {
      headers: {'x-access-token': token},
    })
    const data = await req.json()
    if(data.status == "ok"){
      setOldScore(data.romanian)
    }else{
      alert('Invalid Token');
    }
  };
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingRomanianScore', {
      method: "POST",
