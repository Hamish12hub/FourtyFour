...
import theFlag from '../photos/flags/Russia.png';
...
function Russian() {
  ...
  const TheLanguage = "Russian"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
 "Privet","Privet","Spasibo","Pozhaluysta","I","v","da","Net","Dobroye utro","Dobryy den",
//11-21
 "Yeda","yaytsa","Moloko","Khleb","Chipsy","Ryby","Shokolad","Pitstsa","Voda","Pivo","Vino",
// 22-29
 "Restoran","Dom","Plyazh","Magazin","Gorod","Plavatel'nyy basseyn","Park","Tualet",
//30-35
 "Schet","Den'gi","Telefon","vegetarianets","Trenirovat'sya","Mashina",
//36-44
 "Odin","Dva","Tri","Chetyre","pyat","Shest","Sem","Vosem","Devyat"
    ]);
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/russian", "");
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingRussianScore', {
      method: "POST",
