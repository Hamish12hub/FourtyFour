...
import theFlag from '../photos/flags/Spain.png';
...
function Spanish() {
  ...
  const TheLanguage = "Spanish"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
'Hola', 'Hola', 'Gracias', 'Por favor', 'Y', 'El/La', 'Si', 'No', 'Buenos dias', 'Buenas tardes',
//11-21
'Comida', 'Huevos', 'Leche', 'Pan', 'Papas fritas', 'Pescado', 'Chocolate', 'Pizza', 'Agua', 'Cerveza', 'Vino',
// 22-29
'Restaurante', 'Casa', 'Playa', 'Tienda', 'Ciudad', 'Piscina', 'Parque', 'Bano',
//30-35
'La cuenta', 'Dinero', 'Telefono', 'Vegetariano', 'Tren', 'Coche',
//36-44
'Uno', 'Dos', 'Tres', 'Cuatro', 'Cinco', 'Seis', 'Siete', 'Ocho', 'Nueve'
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/spanish", "");
  ...
  const alertCACScore = async () => {
    ...
    const req = await fetch('http://localhost:1337/SpanishScore', {
      headers: {'x-access-token': token},
    })
    const data = await req.json()
    if(data.status == "ok"){
      setOldScore(data.spanish)
    }else{
      alert('Invalid Token');
    }
  };
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingSpanishScore', {
      method: "POST",
