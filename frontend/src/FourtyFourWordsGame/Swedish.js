...
import theFlag from '../photos/flags/Sweden.png';
...
function Swedish() {
  ...
  const TheLanguage = "Swedish"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
'Halla', 'Hej', 'Tack', 'Vanligen', 'Och', 'Den', 'Ja', 'Nej', 'God morgon', 'God eftermiddag',
//11-21
'Mat', 'Ägg', 'Mjölk', 'Bröd', 'Chips', 'Fisk', 'Choklad', 'Pizza', 'Vatten', 'Ol', 'Vin',
// 22-29
'Restaurang', 'Hus', 'Strand', 'Butik', 'Stad', 'Simbassang', 'Park', 'Toalett',
//30-35
'Notan', 'Pengar', 'Telefon', 'Vegetarian', 'Tag', 'Bil',
//36-44
'Ett', 'Tva', 'Tre', 'Fyra', 'Fem', 'Sex', 'Sju', 'Atta', 'Nio'
    ]);
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/swedish", "");
  ...
  const alertCACScore = async () => {
    ...
    const req = await fetch('http://localhost:1337/SwedishScore', {
      headers: {'x-access-token': token},
    })
    const data = await req.json()
    if(data.status == "ok"){
      setOldScore(data.swedish)
    }else{
      alert('Invalid Token');
    }
  };
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingSwedishScore', {
      method: "POST",
