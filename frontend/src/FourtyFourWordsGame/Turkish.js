...
import theFlag from '../photos/flags/Turkey.png';
...
function Turkish() {
  ...
  const TheLanguage = "Turkish"

       const [score, setScore] = useState(-1);
    const [myOtherLanguage, setMyOtherLanguage] = useState([
//1-10
'Merhaba', 'Selam', 'Tesekkur ederim', 'Lutfen', 'Ve', 'O', 'Evet', 'Hayir', 'Gunaydin', 'Iyi gunler',
//11-21
'Yiyecek', 'Yumurta', 'Sut', 'Ekmek', 'Cips', 'Balik', 'Cikolata', 'Pizza', 'Su', 'Bira', 'Sarap',
// 22-29
'Restoran', 'Ev', 'Plaj', 'Magaza', 'Sehir', 'Yuzme havuzu', 'Park', 'Tuvalet',
//30-35
'Hesap', 'Para', 'Telefon', 'Vejetaryen', 'Tren', 'Araba',
//36-44
"Bir", "Iki", "Uc", "Dort", "Bes", "Alti", "Yedi", "Sekiz", "Dokuz"
  ...
  const navigateBackHome = () => {
    ...
    const newUrl = currentUrl.replace("/turkish", "");
  ...
  const alertCACScore = async () => {
    ...
    const req = await fetch('http://localhost:1337/TurkishScore', {
      headers: {'x-access-token': token},
    })
    const data = await req.json()
    if(data.status == "ok"){
      setOldScore(data.turkish)
    }else{
      alert('Invalid Token');
    }
  };
  ...
  const UpdatingCACScore = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/UpdatingTurkishScore', {
      method: "POST",
