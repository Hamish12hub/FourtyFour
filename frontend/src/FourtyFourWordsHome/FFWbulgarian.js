...
import myImage from '../photos/flags/Bulgaria.png'

import { useNavigate } from "react-router-dom";

function FFWbulgarian() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?bulgarian");
  }


  const deletingBulgarian = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingBulgarian', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingBulgarianScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Bulgarian</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingBulgarian}><RiDeleteBin6Line/></button>
