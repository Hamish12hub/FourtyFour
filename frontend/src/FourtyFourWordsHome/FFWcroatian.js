...
import myImage from '../photos/flags/Croatia.png'

import { useNavigate } from "react-router-dom";

function FFWcroatian() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?croatian");
  }


  const deletingCroatian = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingCroatian', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingCroatianScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Croatian</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingCroatian}><RiDeleteBin6Line/></button>
