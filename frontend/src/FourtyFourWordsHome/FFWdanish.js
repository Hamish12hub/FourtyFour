...
import myImage from '../photos/flags/Denmark.png'
...
function FFWdanish() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?danish");
  }


  const deletingDanish = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingDanish ', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingDanishScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
  ...
  return (
    <div className="FFWOutside">
      <div className="FFWTop">
        ...
        <button className="FFWTopThreeButton" onClick={deletingDanish}><RiDeleteBin6Line/></button>
