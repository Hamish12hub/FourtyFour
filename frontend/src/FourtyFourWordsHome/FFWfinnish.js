...
import myImage from '../photos/flags/Finland.png'
...
function FFWfinnish() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?finnish");
  }


  const deletingFinnish = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingFinnish', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingFinnishScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Finnish</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingFinnish}><RiDeleteBin6Line/></button>
