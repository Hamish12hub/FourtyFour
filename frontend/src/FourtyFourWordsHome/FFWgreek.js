...
import myImage from '../photos/flags/Greece.png'
...
function FFWgreek() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?greek");
  }


  const deletingGreek = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingGreek', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingGreekScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Greek</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingGreek}><RiDeleteBin6Line/></button>
