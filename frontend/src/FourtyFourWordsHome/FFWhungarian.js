...
import myImage from '../photos/flags/Hungary.png'

import { useNavigate } from "react-router-dom";

function FFWhungarian() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?hungarian");
  }


  const deletingHungarian = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingHungarian', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingHungarianScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Hungarian</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingHungarian}><RiDeleteBin6Line/></button>
