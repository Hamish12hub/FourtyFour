...
import myImage from '../photos/flags/Italian.png'
...
function FFWitalian() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?italian");
  }


  const deletingItalian = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingItalian', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingitalianScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Italian</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingItalian}><RiDeleteBin6Line/></button>
