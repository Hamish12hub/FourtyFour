...
import myImage from '../photos/flags/Poland.png'
...
function FFWpolish() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?polish");
  }


  const deletingPolish = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingPolish', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingpolishScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Polish</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingPolish}><RiDeleteBin6Line/></button>
