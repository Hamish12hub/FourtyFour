...
import myImage from '../photos/flags/Romania.png'

import { useNavigate } from "react-router-dom";

function FFWromanian() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?romanian");
  }


  const deletingRomanian = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingRomanian', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingRomanianScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Romanian</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingRomanian}><RiDeleteBin6Line/></button>
