...
import myImage from '../photos/flags/Spain.png'
...
function FFWspanish() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?spanish");
  }


  const deletingSpanish = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingSpanish', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingSpanishScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Spanish</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingSpanish}><RiDeleteBin6Line/></button>
