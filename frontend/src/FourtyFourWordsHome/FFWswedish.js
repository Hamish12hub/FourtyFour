...
import myImage from '../photos/flags/Sweden.png'
...
function FFWswedish() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?swedish");
  }


  const deletingSwedish = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingSwedish', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingSwedishScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Swedish</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingSwedish}><RiDeleteBin6Line/></button>
