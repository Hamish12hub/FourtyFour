...
import myImage from '../photos/flags/Turkey.png'
...
function FFWturkish() {
  ...
  const sheetButton = () => {
    navigate("languageAnswers?turkish");
  }


  const deletingTurkish = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/DeletingTurkish', {
      method: "POST",
  ...
  const ResetButton = async (e) => {
    const token = localStorage.getItem('token');
    const req = await fetch('http://localhost:1337/resetingTurkishScore', {
      method: "POST",
      headers: {"content-Type": "application/json", 'x-access-token' : token},
    })
    const data = await req.json()
    ...
    <div className="FFWOutside">
      <div className="FFWTop">
        <div className="FFWTopOne">
          <p className="FFWTopText">Turkish</p>
        ...
        <button className="FFWTopThreeButton" onClick={deletingTurkish}><RiDeleteBin6Line/></button>
